	...
	--check if @fromtable contains t_letter (this doesn't catch t_LetterLog)
	   if charindex(' t_Letter ', @FromTable) = 0 
		begin --if it doesn't, then check if the table is present already in our final from sentence (else skip this part)
		 ...
		 /*
		 we look in @Where for a <= , which if present means that there is another set of datetimes present
		 if there is another set of datetimes present in the where clause, we need an and seperator
		 */
		  ...
		  if charindex('<=', @Where) > 0 
		   ...
		   --now let our where sentence contain the needed column and data, needed to get the dates we want to see
